...
class SomtodayCard extends LitElement {
  ...
  _renderWeek(student) {
    const state = this._weekChoice === "next"
  ...
  _printWeek(days, student) {
    ...
    popup.document.write(`<!doctype html><html><head><title>${escapeHtml(this._config.title)} – ${escapeHtml(this._t(this._weekChoice === "this" ? "this_week" : "next_week"))}</title><style>@page{size:landscape;margin:12mm}body{font:12px Arial;color:#111}h1{margin:0 0 4px}p{margin:0 0 14px;color:#555}table{width:100%;border-collapse:collapse;table-layout:fixed}th,td{border:1px solid #999;padding:6px;vertical-align:top}thead th{background:#e8eef5}tbody th{width:70px;background:#f3f3f3}small{display:block;margin-top:3px;color:#555}.empty{color:#999;background:#fafafa}.cancelled{text-decoration:line-through;background:#ffe8e8}</style></head><body><h1>${escapeHtml(this._config.title)}</h1><p>${escapeHtml(student.name || "")} · ${escapeHtml(this._t(this._weekChoice === "this" ? "this_week" : "next_week"))}</p><table><thead><tr><th>Uur</th>${days.map((day) => `<th>${escapeHtml(formatDate(day.date, this._locale()))}</th>`).join("")}</tr></thead><tbody>${cells}</tbody></table><script>window.onload=()=>{window.print()}<\/script></body></html>`);
